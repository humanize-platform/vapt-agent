            ...
            async with session.request(
                ...
                if any(indicator in response_text.lower() for indicator in error_indicators):
                    results.append(VAPTTestResult(
                        ...
                        evidence={"payload": payload, "status_code": response.status}
                    ))
...
async def vapt_security_test(args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async with aiohttp.ClientSession() as session:
        ...
        if "injection" in test_types:
            sql_results = await test_sql_injection(session, endpoint, method, headers, body)
        ...
        if "headers" in test_types:
            header_results = await test_security_headers(session, endpoint, method, headers)
    ...
    report = {
        ...
        "total_vulnerabilities": len([r for r in all_results if r.status == "vulnerable"]),
